...
static void		get_map_size(Server *server, String *cmd);
static void		get_tile_info(Server *server, String *cmd);
static void		get_players_by_team(Server *server, String *cmd);
...
void			check_cmd_gui(Server *server, t_socket *socket)
{
  ...
  if (client_cmd[index].name && to_check)
    client_cmd[index].func(server, newString(cmd));
  else
...
static void		get_players_by_team(Server *server, String *cmd)
{
  ...
  while ((it = server->players->loop(server->players)))
    if (strcmp(it->data->team->__str, tmp) == 0)
      out->add(out, newString(" "))->add(out, it->data->name);
  if (out->len(out) <= 1)
